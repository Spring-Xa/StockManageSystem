...
public class CarRoom extends JPanel {
	...
	public CarRoom() {
		...
		panel_9.setBorder(new TitledBorder(null, "车间信息", TitledBorder.LEADING, TitledBorder.TOP, null, null));
		panel_9.setBounds(10, 5, 841, 70);
		this.add(panel_9);
		panel_9.setLayout(new FlowLayout(FlowLayout.LEFT, 5, 5));

		JLabel lblNewLabel = new JLabel("车间号");
		...
		JLabel lblNewLabel_1 = new JLabel("车间名");
		...
		JLabel lblNewLabel_2 = new JLabel("车间描述");
		...
		panel_10.setBorder(new TitledBorder(null, "车间操作", TitledBorder.LEADING, TitledBorder.TOP, null, null));
		FlowLayout flowLayout = (FlowLayout) panel_10.getLayout();
		flowLayout.setAlignment(FlowLayout.LEFT);
		panel_10.setBounds(10, 85, 841, 70);
		this.add(panel_10);

		JLabel lblNewLabel_3 = new JLabel("车间号");
		...
		btnNewButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				String descrip = textField_2.getText();

				// 以上注册信息不能为空
				if (id.equals("")) {
					Tools.showMessage("增加信息", "车间号不能为空");
				} else if (namez.equals("")) {
					Tools.showMessage("增加信息", "车间名不能为空");
				} else if (descrip.equals("")) {
					Tools.showMessage("增加信息", "车间描述不能为空");
				} else {
					// 以上条件都需要满足
					// 插入语句
					String sql = "INSERT INTO d_car_room(id,namez,descrip) VALUES (?,?,?)";
					int a = Mysql.update(sql, id, namez, descrip);
					System.out.println(a);
					// 根据返回值判断状态
					if (a == -1) {
						Tools.showMessage("增加消息", "当前车间已存在，请重新增加");
					}
					if (a == 1) {
		...
		btnNewButton_1.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (textField_3.getText().equals("")) {
					Tools.showMessage("系统提示", "请输入车间号");
				} else {
					String sql = "delete from d_car_room where id=?";
					int a = Mysql.update(sql, textField_3.getText());
					if (a == 1) {
						Tools.showMessage("系统提示", "删除成功");
					}
					if (a == 0) {
						Tools.showMessage("系统提示", "删除失败,车间不存在");
					}
					if (a == -1) {
						Tools.showMessage("系统提示", "删除失败,车间不存在");
					}
		...
		btnNewButton_2.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (textField_3.getText().equals("")) {
					Tools.showMessage("系统提示", "更改车间号不能为空");
				} else if (textField.getText().equals("")) {
					Tools.showMessage("系统提示", "车间号不能为空");
				} else if (textField_1.getText().equals("")) {
					Tools.showMessage("系统提示", "车间名不能为空");
				} else if (textField_2.getText().equals("")) {
					Tools.showMessage("系统提示", "车间描述不能为空");
				} else {
					String sql = "UPDATE d_car_room set id=?,namez=?,descrip=? where id=?";
		...
		btnNewButton_3.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (textField_3.getText().equals("")) {
					String sql = "select * from d_car_room";
					ResultSet rs = Mysql.query(sql, null);
					// 用一个工具自动插入到表格
					Tools.addTable(rs, model, 3);
				} else {
					String sql = "select * from d_car_room where id=?";
